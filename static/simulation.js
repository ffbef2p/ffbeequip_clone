...
var enemyResist = {"fire":0,"ice":0,"water":0,"wind":0,"lightning":0,"earth":0,"light":-50,"dark":0};
var enemyRaces;
...
function buildUnit() {
    ...
    readenemyResists();
    enemyRaces = getSelectedValuesFor("races");
...
function getKillerCoef(item) {
    ...
    if (enemyRaces.length > 0 && item.killers) {
        for (var killerIndex in item.killers) {
            if (enemyRaces.includes(item.killers[killerIndex].name) && item.killers[killerIndex][applicableKillerType]) {
                cumulatedKiller += item.killers[killerIndex][applicableKillerType];
            }
        }
    }
    return cumulatedKiller / enemyRaces.length;
}

function readenemyResists() {
    for(var element in enemyResist) {
        var value = $("#elementalResists td." + element + " input").val();
        if (value) {
            enemyResist[element] = parseInt(value);
        } else {
...
function getDamageCoefLevel(item) {
    ...
    if (builds[currentUnitIndex].statToMaximize == "atk" || builds[currentUnitIndex].statToMaximize == "mag") {
        ...
        if (weaponList.includes(item.type) && useWeaponsElements) {
            // only for weapons
            if ((item.element && enemyResist[item.element] != 0)) {
...
function getWeaponElementDamageCoef(weaponElements) {
    ...
    for (var elementIndex in weaponElements) {
        value += enemyResist[weaponElements[elementIndex]];
    }
...
function calculateBuildValue(equiped, esper) {
    if ("atk" == builds[currentUnitIndex].statToMaximize || "mag" == builds[currentUnitIndex].statToMaximize) {
        ...
        if (elements.length > 0) {
            for (var element in enemyResist) {
                if (elements.includes(element)) {
                    resistModifier += enemyResist[element] / 100;
                }
            }    
        ...
        if (enemyRaces.length > 0) {
            killerMultiplicator += (cumulatedKiller / 100) / enemyRaces.length;
        }
...
function calculateBuildKillerValue(killerType) {
        ...
        for (var equipedIndex in itemAndPassives) {
            if (itemAndPassives[equipedIndex] && (areConditionOK(itemAndPassives[equipedIndex], equiped))) {
                if (currentEnemyRaces.length > 0 && itemAndPassives[equipedIndex].killers) {
                    for (var killerIndex = 0; killerIndex <  itemAndPassives[equipedIndex].killers.length; killerIndex++) {
                        if (currentEnemyRaces.includes(itemAndPassives[equipedIndex].killers[killerIndex].name) && itemAndPassives[equipedIndex].killers[killerIndex][killerType]) {
                            cumulatedKiller += itemAndPassives[equipedIndex].killers[killerIndex][killerType];
                        }
                    }
                }
            }
...
function onRacesChange() {
    var physicalKillerValue = calculateBuildKillerValue("physical");
    var magicKillerValue = calculateBuildKillerValue("magical");
    $("#unit" + (currentUnitIndex + 1) + "Current_killerPhy").val(physicalKillerValue);
    $("#unit" + (currentUnitIndex + 1) + "Current_killerMag").val(magicKillerValue);
}
...
function getStateHash() {
    ...
    if (data.goal == "atk" || data.goal == "mag") {
        data.enemyRaces = getSelectedValuesFor("races");
        readenemyResists();
        data.enemyResists = enemyResist;
    }
...
function loadStateHashAndBuild(data) {
    ...
    if (data.goal == "atk" || data.goal == "mag") {
        select("races", data.enemyRaces);
        for (var element in data.enemyResists) {
            if (data.enemyResists[element] == 0) {
                $("#elementalResists ." + element + " input").val("0");
            } else {
                $("#elementalResists ." + element + " input").val(data.enemyResists[element]);
            }
        }
    }
...
function formatHtmlElementList(elementList) {
    var htmlText = "";
    var firstElement = true;
    if (elementList) {
        elementList.forEach(function(element) {
            htmlText = htmlText.concat("<span class=attackElement style=color:" + elementsColorMap[element] + ">");
            if (firstElement) {
                firstElement = false;
            } else {
                htmlText = htmlText.concat(" ");
            }
            htmlText = htmlText.concat(element + "</span>");
        });
    }
    return htmlText;
}
...
function unitAttack(unitIndex) {
    ...
    if (selectedSkill == null) {
        ...
        switch (selectedSkill.atkType.toLowerCase()) {
            case "physical": {
                var attackResult = physicalAttack(unitIndex, selectedSkill, "unit" + unitIndex + "Current_", "monsterCurrent_", selectedSkill.multiplier, selectedSkill.ignoreDef);
                damageRange = attackResult[0];
                elementList = attackResult[1];
            }
            break;
            case "magic":
                var attackResult = magicAttack(unitIndex, selectedSkill, "unit" + unitIndex + "Current_", "monsterCurrent_", selectedSkill.multiplier, selectedSkill.ignoreSpr);
                damageRange = attackResult[0];
                elementList = attackResult[1];
            break;
            case "hybrid":
                var attackResult = hybridAttack(unitIndex, selectedSkill, "unit" + unitIndex + "Current_", "monsterCurrent_", selectedSkill.multiplier, selectedSkill.ignoreDef, selectedSkill.ignoreSpr);
                damageRange = attackResult[0];
                elementList = attackResult[1];
            break;
            case "fixed":
    ...
    var elementListHtml = formatHtmlElementList(elementList);
    if (elementListHtml.length > 0) {
        elementListHtml = "<span class=elementList> {" + elementListHtml + "}</span>";
    }

    if (damageRange) {
        damage = damageRange[0];
        $("#unit" + unitIndex + "Atk .damageResult").html(damage + " [" + damageRange[1] + " to " + damageRange[2] + "]" + elementListHtml );
    } else {
        $("#unit" + unitIndex + "Atk .damageResult").html(damage + elementListHtml);
    }
...
function findAttackElements(unitIndex) {
    var attackElements = [];
    if (builds[unitIndex - 1].bestBuild[0] && builds[unitIndex - 1].bestBuild[0].element) {
        builds[unitIndex - 1].bestBuild[0].element.forEach(function(element) {
            attackElements.push(element)
        });
    }
    
    if (builds[unitIndex - 1].bestBuild[1] && builds[unitIndex - 1].bestBuild[1].element) {
        builds[unitIndex - 1].bestBuild[1].element.forEach(function(element) {
            attackElements.push(element)
        });
    }
...
function calculateElementMultiplier(unitIndex, selectedSkill) {
    var elementList = [];
    var resistList = [];

    if (selectedSkill) {
        if (selectedSkill.elemental) {
            var skillElements = selectedSkill.elemental.split(",");
            skillElements.forEach(function(element) {
                elementList.push(element.toLowerCase());
            });
        }
    }
    
    if (!selectedSkill || (selectedSkill.atkType.toLowerCase() == "physical")) {
        var attackElements = findAttackElements(unitIndex);
        attackElements.forEach(function(element) {
    ...
    elementList.forEach(function(element) {
        var resist = parseInt($("#elementalResists td." + element + " input").val());
//                    if (selectedSkill["lowerResist" + attackElements[element]]) {
//                        resist += selectedSkill["lowerResist" + attackElements[element]];
//                    }
        resistList.push([element, resist]);
        if (!resultElementList.includes(element)) {
    ...
    if (resistList.length > 0) {
        for (var element in resistList) {
            resistSum += resistList[element][1];
        }
        var multiplier = (1 - ((resistSum / resistList.length) / 100));
        if (multiplier < 0) {
            multiplier = 0;
        }
        return [ multiplier, resultElementList ];
    }
    return [ 1 , [] ];
}

function singleHandAttack(unitIndex, selectedSkill, atkUnitPrefix, defUnitPrefix, multiplier, ignoreDef, isLeftHand) {
    ...
    if (currentEnemyRace.length > 0) {
        killerValue = (1 + ((parseInt($("#" + atkUnitPrefix + "killerPhy").val()) / 100) / currentEnemyRace.length));
    }
    ...
    var damage = 0;
    var actualAtk = atk;
    if (builds[unitIndex - 1].bestBuild[0] && builds[unitIndex - 1].bestBuild[1]) {
        if (isLeftHand) {
            if (builds[unitIndex - 1].bestBuild[1]["atk"]) {
                actualAtk -= builds[unitIndex - 1].bestBuild[1]["atk"]; // minus right hand
            }
        } else {
            if (builds[unitIndex - 1].bestBuild[0]["atk"]) {
    ...
    return [Math.floor(damage), Math.floor(damageMin), Math.floor(damageMax), elementMultiplierResult[1]];
}
...
function magicAttack(unitIndex, selectedSkill, atkUnitPrefix, defUnitPrefix, multiplier, ignoreSpr) {
    ...
    var currentEnemyRace = getSelectedValuesFor("races");
    if (currentEnemyRace.length > 0) {
        killerValue = (1 + ((parseInt($("#" + atkUnitPrefix + "killerMag").val()) / 100) / currentEnemyRace.length));
    }
    ...
    if (selectedSkill.chargeIncremental) {
        if (!$("#unit" + unitIndex + "Current_stackCharge").is('[disabled=disabled]')) {
            var charge = $("#unit" + unitIndex + "Current_stackCharge").val();
            if (charge > 0) {
                if (charge > selectedSkill.chargeMax) {
                    charge = selectedSkill.chargeMax;
                }
                multiplier = multiplier + (selectedSkill.chargeIncremental * charge);
            }
        }
    }
...
//    alert (mag + ", " + spr + ", " + multiplier + ", " + levelCorrection + ", " +  elementMultiplierResult[0] + ", " + killerValue)
    ...
    var damageRange = [Math.floor(damage), Math.floor(damageMin), Math.floor(damageMax)];
        
    return [ damageRange, elementMultiplierResult[1] ];
}
...
function singleHandHybridAttack(unitIndex, selectedSkill, atkUnitPrefix, defUnitPrefix, multiplier, ignoreSpr, ignoreDef, isLeftHand) {
    var physicalDamageRange = singleHandAttack(unitIndex, selectedSkill, atkUnitPrefix, defUnitPrefix, multiplier[0], ignoreDef, isLeftHand);
    var magicDamageRange = magicAttack(unitIndex, selectedSkill, atkUnitPrefix, defUnitPrefix, multiplier[1], ignoreSpr);

    var damageRange = [ 0, 0, 0 ];
    for (var damageRangeIndex in damageRange) {
        damageRange[damageRangeIndex] = (physicalDamageRange[0][damageRangeIndex] + magicDamageRange[0][damageRangeIndex] / 2);
    }
    return [ damageRange, unionArrays(physicalDamageRange[3], magicDamageRange[1]) ];
}

function hybridAttack(unitIndex, selectedSkill, atkUnitPrefix, defUnitPrefix, multiplier, ignoreSpr, ignoreDef) {
    var physicalDamageRange = physicalAttack(unitIndex, selectedSkill, atkUnitPrefix, defUnitPrefix, multiplier[0], ignoreDef);
    var magicDamageRange = magicAttack(unitIndex, selectedSkill, atkUnitPrefix, defUnitPrefix, multiplier[1], ignoreSpr);
